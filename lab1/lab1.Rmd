...
Before we can use any packages we have to 'load' them into our R session. We do that by calling  the `library()` function with the name of the package as the only argumet. 
...
The results tell us that we have over 9 thousand observations and 13 variables. It tells us the variable names, their type and the first few observations. We see that variable Date is a `<date>`, and the rest of the variables are `<dbl>` which means that they are numbers stored with 'double' precision.
...
### IN-CLASS EXERCISE
1. Load in data on the results of the 2016 NHL season from https://www.dropbox.com/s/krlwr6z38ol6wer/NHLseason2016.csv?raw=1 or from 
...
1. Plot the density of attendance.

***
***
## Exercises
Write a new R Markdown file that does the analysis and answers the questions below. Knit your R Markdown, print it and bring to class. Don't forget to load `tidyverse` at the beginning of your code.
...
4. Plot the adjusted closing prices GE over time. Use [this documentation for ggplot2](http://rstudio-pubs-static.s3.amazonaws.com/3364_d1a578f521174152b46b19d0c83cbe7e.html) to add title to your plot.   
5. Use [the same documentation](http://rstudio-pubs-static.s3.amazonaws.com/3364_d1a578f521174152b46b19d0c83cbe7e.html) to create a new plot that uses logarithmic scale on the y axis. Add pretty labels on the y axis ('breaks' in `ggplot` terminology). E.g. 100,200,400 etc.  
